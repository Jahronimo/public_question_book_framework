...
def TOP_PAGE_INFORMATION():
    ...
    (num_question_to_display), ("questions with the subject interest of your choice ").upper(), (topic_of_interest))
    print (question_sections_edge_dividers_for_bottom_of_page)
    #______________________________________________________________________________________________________________
...
def Randomise(questions_lists):
    ...
    if num_question_to_display > len(questions_lists):
        #num_question_to_display = len(questions_lists)
        pass
...
R_INTRO = random.choices(intros_to_questions)[0]
R_TOPIC = "regarding " + random.choices(topic_of_interest)[0]
R_TIME = random.choices(time_scales)[0] 
R_ANSWER = random.choices(how_to_answer_questions)[0] 
R_NEG_EMOTIONS = random.choices(negitive_emotions)[0] 
R_POS_EMOTIONS = random.choices(places_to_focus_on)[0] 
R_PLACES = random.choices(places_to_focus_on)[0] 
R_PEOPLE = random.choices(groups_of_people)[0] 
...
class Self_Assessment(bookbase):
    ...
    def define_the_problem_Self_Assessment(self):
        ...
        questions_lists = [
        (f"{R_INTRO} What might be the biggest problem {R_TOPIC}?" 
        ...
        f"your goals in the last {R_TIME} {R_TOPIC}?" 
        f"{R_ANSWER}"),

        (f"{R_INTRO}, When {R_TOPIC}, Have you met your expectations"
        f"(why or why not?) {R_ANSWER}"),

        (f"{R_INTRO} Where are you struggling most {R_TOPIC}? "
        ...
        f"towards my goal {R_TOPIC}? {R_ANSWER}"),

        (f"{R_INTRO} Who is causing me most problems {R_TOPIC}?"
        ...
        (f"{R_INTRO} What is my often main focus {R_TOPIC}?, "
        ...
        f" is this helping me {R_TOPIC}?, {R_ANSWER}"),

        (f"{R_INTRO} Does a problem I am facing need an immediate solution "
        f" {R_TOPIC}? or can it wait? {R_ANSWER}".format(R_INTRO,R_TOPIC,R_ANSWER)),

        (f"{R_INTRO} Have I ever been warned or told of any problems "
        f"that I might face {R_TOPIC}? What could be the truth to it? "
        f"{R_ANSWER}"),

        (f"{R_INTRO} What problems {R_TOPIC} have I been refusing to acknowledge "
        ...
        f"or said, to taking the next positive step  {R_ANSWER}?"
        f" {R_INTRO}"),

        (f"{R_INTRO} Where is causing me unnecessary Stress "
        f" {R_TOPIC}? {R_ANSWER}"),

        (f"{R_INTRO} what is causing me necessary positive Stress {R_TOPIC}?"
        f" {R_INTRO}"),

        (f"{R_INTRO} Where are you inefficiently focusing on {R_TOPIC}?"
        f" {R_ANSWER}"),

        (f"{R_INTRO} What am I doing that is counter productive {R_TOPIC}?"
        f" {R_ANSWER}"),

        (f"{R_INTRO} Where is my goal being badly defined {R_TOPIC}?"
        f"What sorts of problems is this creating?"
        f" {R_ANSWER}"),

        (f"{R_INTRO} How can I identify and create less daily problematic "
        f"resistances {R_TOPIC}? "
    ...
    def Self_Assessment_1(self):
        ...
        questions_lists = [
        ("{} who          {}? {}".format(R_INTRO,R_TOPIC,R_ANSWER)),
        ("{} What          {}? {}".format(R_INTRO,R_TOPIC,R_ANSWER)),
        ("{} Where          {}? {}".format(R_INTRO,R_TOPIC,R_ANSWER)),
        ("{} When          {}? {}".format(R_INTRO,R_TOPIC,R_ANSWER)),  
        ("{} When          {}? {}".format(R_INTRO,R_TOPIC,R_ANSWER)), 
        ]
        ...
        print("if looking for answers, what will happen when I ask the right questions  {}?".format(topic_of_interest))
        print("Would I like to import  {}".format(topic_of_interest))
        print("I am aware there are thing to be improved upon  {}?".format(topic_of_interest))
        # example of multiple formattings using the same string
        print("this is showing {} multiple formattings of the same string {}".format(topic_of_interest,topic_of_interest))
